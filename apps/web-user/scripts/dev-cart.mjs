...
function start(command, args, cwd, label) {
  ...
  child.once("exit", (code) => {
    ...
    if (!shuttingDown && code && code !== 0) {
      console.error(`[dev:cart] ${label} stopped with exit code ${code}.`);
      shutdown(code);
    }
  });
  return child;
...
async function startMock(service) {
  if (await isPortOpen(service.port)) {
    console.log(`[dev:cart] Reusing ${service.name} on port ${service.port}.`);
    return;
  }
  console.log(`[dev:cart] Starting ${service.name} on port ${service.port}...`);
  const dataPath = path.join("mocks", "mockoon", service.file);
  const child = start(
    "npx",
    ["@mockoon/cli", "start", "--disable-log-to-file", "--data", dataPath],
    repositoryRoot,
    service.name
  );
...
try {
  ...
  if (await isPortOpen(3000)) {
    console.log("[dev:cart] Reusing Next.js on http://localhost:3000.");
    console.log("[dev:cart] Product Detail and Cart are ready. Press Ctrl+C to stop mock services started by this command.");
    process.stdin.resume();
  } else {
    console.log("[dev:cart] All mock services are ready. Starting Next.js...");
    const web = start("npm", ["run", "dev"], appDirectory, "Next.js");
    web.once("exit", (code) => shutdown(code ?? 0));
  }
} catch (error) {
  console.error(`[dev:cart] ${error instanceof Error ? error.message : String(error)}`);
  shutdown(1);
}
