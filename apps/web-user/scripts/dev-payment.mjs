...
const mockServices = [
  ...
  { name: "Payment Mockoon", port: 4016, file: "oma-payment-mvp.json" }
];
...
function start(command, args, cwd, label) {
  ...
  child.once("exit", (code) => {
    ...
    if (!shuttingDown && code && code !== 0) {
      console.error(`[dev:payment] ${label} stopped with exit code ${code}.`);
      shutdown(code);
    }
  });
  return child;
...
async function startMock(service) {
  if (await isPortOpen(service.port)) {
    console.log(`[dev:payment] Reusing ${service.name} on port ${service.port}.`);
    return;
  }
  console.log(`[dev:payment] Starting ${service.name} on port ${service.port}...`);
...
try {
  ...
  if (await isPortOpen(3000)) {
    console.log("[dev:payment] Reusing Next.js on http://localhost:3000.");
    console.log("[dev:payment] Cart, Checkout and Payment are ready. Press Ctrl+C to stop services started by this command.");
    process.stdin.resume();
  } else {
    console.log("[dev:payment] All mock services are ready. Starting Next.js...");
    const web = start("npm", ["run", "dev"], appDirectory, "Next.js");
    web.once("exit", (code) => shutdown(code ?? 0));
  }
} catch (error) {
  console.error(`[dev:payment] ${error instanceof Error ? error.message : String(error)}`);
  shutdown(1);
}
