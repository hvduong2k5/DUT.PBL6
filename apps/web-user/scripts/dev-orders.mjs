...
function start(command, args, cwd, label) {
  ...
  child.once("exit", (code) => {
    ...
    if (!shuttingDown && code && code !== 0) {
      console.error(`[dev:orders] ${label} stopped with exit code ${code}.`);
      shutdown(code);
    }
  });
  return child;
...
async function startMock(service) {
  if (await isPortOpen(service.port)) {
    console.log(`[dev:orders] Reusing ${service.name} on port ${service.port}.`);
    return;
  }
  console.log(`[dev:orders] Starting ${service.name} on port ${service.port}...`);
...
try {
  ...
  if (await isPortOpen(3000)) {
    console.log("[dev:orders] Reusing Next.js on http://localhost:3000.");
    console.log("[dev:orders] Order Management is ready. Press Ctrl+C to stop services started by this command.");
    process.stdin.resume();
  } else {
    console.log("[dev:orders] All mock services are ready. Starting Next.js...");
    const web = start("npm", ["run", "dev"], appDirectory, "Next.js");
    web.once("exit", (code) => shutdown(code ?? 0));
  }
} catch (error) {
  console.error(`[dev:orders] ${error instanceof Error ? error.message : String(error)}`);
  shutdown(1);
}
